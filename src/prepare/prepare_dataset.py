...
class PrepareDataset:
    ...
    def __call__(self) -> None:
        ...
        for i, recording in enumerate(recordings):
            ...
            if dot_info is None:
                continue
            face_grid_info = self.read_json(
                recording_dir.joinpath(fs.FACE_GRID_JSON))
            if face_grid_info is None:
                continue
            frames_info = self.read_json(recording_dir.joinpath(
                fs.FRAMES_JSON))
            ...
            for index, frame_index in enumerate(frames_info):
                ...
                # Collect metadata
                metadata[md.RECORDING_INDEX].append(recording_index)
                metadata[md.FRAME_INDEX].append(frame_index)
                metadata[md.GAZE_X].append(dot_info['XCam'][index])
                metadata[md.GAZE_Y].append(dot_info['YCam'][index])
                metadata[md.FACE_GRID].append(face_grid_bbox[index, :])
    ...
    def split_metadata(self,
                       ...
                       ratio: float = 90) -> pd.DataFrame:
        randnums = np.random.randint(1, 100, metadata.shape[0])
        split = [md.TRAIN if x < ratio else md.VALIDATE for x in randnums]
